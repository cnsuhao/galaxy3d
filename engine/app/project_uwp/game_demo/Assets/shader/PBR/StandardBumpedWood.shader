PBR/StandardBumpedWood
{
	...
	HLPS ps
	{
		...
		Texture2D _UVOffsetMap : register(t3);
		SamplerState _UVOffsetMap_Sampler : register(s3);
		Texture2D _RoughnessMap : register(t4);
		SamplerState _RoughnessMap_Sampler : register(s4);
		...
		{
			float plank_scale = 1.0;
			float wood_scale = 1.0;

			float2 uv = input.v_uv * plank_scale;
			uv = uv * 0.5;
			float4 uv_offset = _UVOffsetMap.Sample(_UVOffsetMap_Sampler, uv);
			uv = (uv_offset.g * 3.33 + input.v_uv.yx + uv_offset.b * 7.773) * wood_scale;
			float Smoothness = lerp(0.5, 0.985, _RoughnessMap.Sample(_RoughnessMap_Sampler, uv).r) - 0.1;

			float4 c = _MainTex.Sample(_MainTex_Sampler, uv) * _Color;
