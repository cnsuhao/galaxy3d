...
Texture2D _MaskMap : register(t2);
SamplerState _MaskMap_Sampler : register(s2);
...
float4 main(PS_INPUT input) : SV_Target
{
	...
	uv = uv * float2(1.0, 0.4) + float2(0.0, 0.15);
	float decal = _MainTex.Sample(_MainTex_Sampler, uv).r;
	if(int(_UseDecal.x) == 0)
	{
		decal = 0;
	}
	float3 emissive = lerp(float3(0, 0, 0), float3(0.858, 0.521, 0), decal);

	float3 masks = _MaskMap.Sample(_MaskMap_Sampler, input.v_uv * _UVScale.x).rgb;
	float rough = lerp(_Roughness.x, _Roughness.y, masks.r);
	rough = lerp(rough, _Roughness.z, masks.g);
	float smoothness = 1 - rough;

	float4 c = lerp(_Color, float4(0, 0, 0, 0), decal);
