PBR/Water
{
	...
	HLPS ps
	{
		...
		TextureCube _ReflectionMap : register(t0);
		SamplerState _ReflectionMap_Sampler : register(s0);
		...
		float4 main(PS_INPUT input) : SV_Target
		{
			...
			float3 normal0 = _NormalMap.Sample(_NormalMap_Sampler, input.v_uv0).rgb;
			float3 normal1 = _NormalMap.Sample(_NormalMap_Sampler, input.v_uv1).rgb;
			normal0 = normal0 * 2.0 - 1.0;
			normal1 = normal1 * 2.0 - 1.0;
			float3 normal = normalize(0.5 * (normal0 + normal1));
			...
			half3 uv_cube = reflect(-eye_dir, normal);
