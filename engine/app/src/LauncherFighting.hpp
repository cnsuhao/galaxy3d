...
class LauncherFighting : public Component
{
	...
	virtual void Start()
	{
		...
		RenderSettings::light_ambient = Color(1, 1, 1, 1) * 0.25f;
		RenderSettings::GetGlobalDirectionalLight()->GetTransform()->SetRotation(Quaternion::Euler(70, 75, 0));
	...
	virtual void Update()
	{
		m_fps->GetLabel()->SetText("fps:" + GTString::ToString(GTTime::GetFPS()) + "\n" +
			...
			g_cam->GetTransform()->GetPosition().ToString() + "\n" +
			g_cam->GetTransform()->GetRotation().ToString()
			);
